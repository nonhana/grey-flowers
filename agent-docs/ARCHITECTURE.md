...
- `src/routes/` — TanStack Router file-based routes: `__root.tsx` mounts the console shell (nav rail covering overview, activities, comments, users, music, assets, articles, taxonomy), one file per page route; `router.ts` wires `createRouter` over the generated `src/routeTree.gen.ts` (committed, regenerated by `@tanstack/router-plugin` with `autoCodeSplitting`).
...
- `src/features/` — one folder per workflow: `overview/`, `activities/`, `comments/`, `users/`, `music/`, `assets/`, `articles/`, `taxonomy/`. List features own a per-feature `search.ts` zod schema consumed by their route's `validateSearch`.
