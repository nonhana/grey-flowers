...
All three apps deploy from `master` via per-app workflows (`deploy-{api,admin,main}.yml`), each gated by the shared reusable `.github/workflows/gate.yml` (fmt/lint/typecheck/test) before building. api ships a self-contained `pnpm deploy` bundle (dist + production node_modules) to `API_DEPLOY_PATH`; admin rsyncs its static `dist/` to `ADMIN_DEPLOY_PATH`; main ships only `apps/main/.output/` to `MAIN_DEPLOY_PATH` and PM2 reloads `grey-flowers` from `.output/server/index.mjs`. `ecosystem.config.cjs` and runtime env are hand-maintained on the server, not in the repo. DB migrations are applied on the server separately (not by the deploy workflows).
