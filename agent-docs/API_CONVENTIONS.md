...
| `AUTH_FORBIDDEN` | 403 | Authenticated but not `ADMIN` |
...
| `INTERNAL_ERROR` | 500 | Unhandled error (logged with the `requestId`) |
...
- Refresh: cookie `gf_refresh`; `POST /auth/refresh` rotates it. Sessions last 30 days; refresh secrets are hashed and stored in `Session`. Only the client's transport layer talks to `/auth/refresh`.
...
| Mount | Auth | Notes |
| --- | --- | --- |
| `/auth/*` | mixed | register/login/refresh/logout (origin-gated), session, me |
...
| `/public/*` | none | `public/articles/*` (list/detail/search/neighbors/dates/count/preview), `public/tags`, `public/categories` |
...
- Conventions already settled in modules:
  ...
  - Save input carries `createSnapshot` / `preserveServerSnapshot` booleans; out-of-date `revision` → `ARTICLE_STALE` (409).
  ...
  - Preview: `POST /articles/:id/preview-token` mints a short-lived token; `GET /public/articles/preview?path=&token=` serves a draft one-shot under `noindex` for the main site's SSR.
...
- Uploads (multipart) go through an XHR path (`upload`) so `onUploadProgress` gets real progress and the same envelope/refresh semantics apply; a `Content-Length` guard in `apps/api` rejects oversize uploads early (`ASSET_PAYLOAD_TOO_LARGE`).
...
Each request gets a `requestId` (`http/middleware/request-id.ts`) surfaced in the envelope and the pino logs (`requestLogger`, `bootstrap/logger.ts`). `handleError` logs `requestId` for unhandled errors so the failure can be correlated.
