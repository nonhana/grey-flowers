...
**The shell.** Desktop is a full-height flex row: a type-case rail (brand block, a create action visually separated from navigation, grouped navigation, an account block pinned to the bottom), then the content column. The rail defaults to 264px, is resizable by dragging its right-edge handle between 208px and 384px, and collapses to a 56px icon column via the toggle at the top-right of the brand row (width and collapsed state persist in localStorage); the right-edge handle doubles as a keyboard-adjustable separator. The height chain is unbroken from `html` / `body` / `#root` down, so the rail reaches the floor rather than ending where its content ends.
...
- **Rail (desktop):** `case` fill, `1px rule` right border; default width 264px, resizable 208–384px via the right-edge drag handle (also a keyboard-operable `separator`), collapsible to a 56px icon column from the toggle at the top-right of the brand row. Brand block, then the create action, then grouped sections, then the account block above a hairline. Collapsed, the rail keeps icons only — the brand row becomes the expand toggle, the create action, navigation rows and the account's sign-out — each with a `title`/`aria-label` fallback; section labels, article-filter sub-rows, the debug delay control and the theme toggle hide. Rows are `40px`, `8px` radius, sans body, `ink-dim` at rest, `accent-wash` + `accent-text` on hover and active — active additionally goes bold. Sub-rows (the article status filters) are mono `0.75rem`, indented, and go `font-medium` on active because mono has no 700.
