...
/** 上传不写进文档（避免污染自动保存）：占位塞进这个 UI-only field 在插入点渲染带进度幽灵图，成功后才把最终 Markdown 插进文档 */
export const uploadField = StateField.define<UploadEntry[]>({
  ...
  update(entries, transaction) {
    ...
    // 上传在途文档被输入改动：占位插入点沿变更平移防漂移；上传自身带起的变更不平移（insertAt 已在 dispatch 时按当前文档给好），同事务移除的 entry 平移无副作用
