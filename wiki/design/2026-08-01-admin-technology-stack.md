...
> **依赖准入规则已触发（2026-08-04）**：`zustand@5` 已成为 `apps/admin` 的全局/领域状态库（播放器、主题、认证为全局单例；文章编辑器为 `createStore` 工厂 + 同文件 hook 绑定），见 `../plans/2026-08-04-admin-zustand-store-refactor.md` §一。触发条件成立依据：播放器与主题原本是手搓 `useSyncExternalStore`（证实的重复），编辑器为绕开 React 闭包问题出现 11 state + 6 ref 镜像 + 5 同步 effect（复杂校验负担）。
