...
const activities = computed(() => fetchData.value?.payload ?? [])
...
<template>
  <HanaInfoCard title="最近动态" icon="lucide:activity">
    <template #action>
      <HanaButton v-if="activities.length > 0" icon="lucide:arrow-right" @click="gotoRecently">
        查看全部动态
      </HanaButton>
    </template>
    <div v-if="activities.length === 0" class="py-12 text-center text-text dark:text-hana-white-700">
